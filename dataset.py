...
class HSIDataset:
    def __init__(self, data_name, pcaComponents, patchsz, train_val_test, random_state=11413):
        super().__init__()
    ...
    def createPatches(self, data, label, patchsz):
        ...
        patched_data = np.zeros((num_sample, patchsz, patchsz, data.shape[2]), dtype=np.float32)
        patched_label = np.zeros(num_sample, dtype=np.int32)
        dx = patchsz // 2
        print(patched_data.shape)
        for i, (x, y) in enumerate(sample_ind):
            patched_data[i] = data[x:x+2*dx+1, y:y+2*dx+1]
